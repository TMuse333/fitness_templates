...
const ExerciseDisplay = ({ workoutData }) => {
  ...
  const handleItemClick = (workoutIndex, exerciseIndex) => {
    const isAlreadyClicked = clickedWorkout.some((item) => (
      item.workoutIndex === workoutIndex && item.exerciseIndex === exerciseIndex
    ));
    ...
    if (isAlreadyClicked) {
      ...
      setClickedWorkout([...clickedWorkout, { workoutIndex, exerciseIndex }]);
    }
  };
