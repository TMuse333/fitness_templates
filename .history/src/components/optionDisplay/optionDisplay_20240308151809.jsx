...
const ProductCarousel = () => {
  ...
  const combinedVariants = (index,button) =>  {
    ...
    hidden: {
      ...
      height: '0px',
      transition: {
        y: { duration: 0.3 }, // Adjust the duration for the y transition
        opacity: { delay: 0.4, duration: 0.5 }, // Add delay for the opacity transition
        height: { duration: 0.4, ease: 'easeInOut' }, // Add transition for the height property
      },
    },
    visible: {
      opacity: 1,
      ...
      transition: {
        ...
        duration: 0.71, // Set the duration to 0.4 seconds for common transitions
        ease: 'easeInOut', // Add a smooth transition easing
  ...
  const productStyles = products.map((product, index) => {
    ...
    return {
      ...
      transition: `left 0.5s ease, transform 0.5s ease, ${
        isCentered ? 'opacity 0.5s ease 0.5s' : 'opacity 0.5s ease' // Apply a 0.5s delay to opacity when centered
      }`,
...
>
              <button
                ...
                style={{
                  ...
                  transition:productStyles[index].tr
                }}
              >
                <FiChevronLeft />
              </button>
