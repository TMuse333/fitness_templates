...
const ProductCarousel = () => {
  ...
  const productStyles = products.map((product, index) => {
    ...
    const shouldTransition =
      (buttonClicked === 'left' && index !== 0) || (buttonClicked === 'right' && index !== 2);
    ...
    return {
      ...
      filter: isCentered ? 'none' : 'brightness(55%)', // Adjust the brightness for non-centered elements
      transition: shouldTransition
        ? `${positions[(currentPosition + index) % positions.length].transition}, left 0.5s ease, transform 0.5s ease`
        : 'none',
    };
  });
