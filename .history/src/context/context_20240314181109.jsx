...
export const WorkoutProvider = ({ children }) => {
  ...
  const [insertData, setInsertData] = useState(false)
    ...
    const calculateProgress = (filteredWorkouts, weeklyProgressSelected, weekSelected) => {
      if (weeklyProgressSelected && weekSelected) {
        ...
        sortedWorkouts.forEach((workout) => {
          workout.exercises.forEach((exercise) => {
            ...
            const existingExerciseIndex = progressData.findIndex((item) => item.name === name);
    
            if (existingExerciseIndex !== -1) {
              // If the exercise exists, add the current workout data to it
              const totalWeight = calculateTotalWeight(exercise.sets);
              progressData[existingExerciseIndex].workouts.push({
