        ...
        if (weekSelected) {
          ...
          progressData.forEach((exercise) => {
            ...
            for (let i = 0; i < exercise.workouts.length - 1; i++) {
              const currentWorkout = exercise.workouts[i];
              const nextWorkout = exercise.workouts[i + 1];
          
              // Calculate the difference between current and next workout
              const weightDiff = nextWorkout.weight - currentWorkout.weight;
              const repsDiff = nextWorkout.reps - currentWorkout.reps;
          
              // Log the progress for each workout
              console.log(`Date: ${currentWorkout.date}`);
              console.log(`Set ${i + 1} to Set ${i + 2}: Weight Diff - ${weightDiff}, Reps Diff - ${repsDiff}`);
            }
          
            // Calculate the difference between the first and last workout
            const firstWorkout = exercise.workouts[0];
            const lastWorkout = exercise.workouts[exercise.workouts.length - 1];
          
            const totalWeightDiff = lastWorkout.weight - firstWorkout.weight;
            const totalRepsDiff = lastWorkout.reps - firstWorkout.reps;
          
            // Log the overall progress for the exercise
            console.log(`Overall Progress: Weight Diff - ${totalWeightDiff}, Reps Diff - ${totalRepsDiff}`);
          });
